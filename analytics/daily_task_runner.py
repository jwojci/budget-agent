...
from analytics.email_processor import EmailProcessor
from analytics.header_validator import HeaderValidator
...
class DailyTaskRunner:
    def __init__(self, app_context: dict):
        ...
        # Initialize services used for the daily run
        self.header_validator = HeaderValidator(self.sheets_service)
        self.monthly_archiver = MonthlyArchiver(
            self.sheets_service, app_context["expense_data_manager"]
        )
        self.email_processor = EmailProcessor(
            self.sheets_service, self.telegram_service, app_context["gmail_service"]
        )
    ...
    async def run_daily_tasks(self, application: Application):
        ...
        try:
            ...
            await self.header_validator.check_and_fix_expenses_header()
